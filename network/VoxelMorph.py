...
import network.utils_vm as layers
from network.args_vm import LoadableModel, store_config_args

'''
VoxelMorph Implmented across the experiments:

https://github.com/voxelmorph/voxelmorph/tree/master/voxelmorph
'''
...
class Unet(nn.Module):
    ...
    def __init__(self, inshape, nb_features=[[16, 32, 32, 32],[32, 32, 32, 32, 32, 16, 16]], nb_levels=None, feat_mult=1):
        super().__init__()
        """
        Parameters:
        ...
        if nb_features is None:
            nb_faetures = default_unet_features()
...
class VxmDense(LoadableModel):
    ...
    def forward(self, source, target, registration=True):
        ...
        '''
        # MW EDIT for segmentation:
        y_target = self.transformer(target, pos_flow)
        return y_target, pos_flow
